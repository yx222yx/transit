...
早期文件名中的 `0.3.0` 只是本地验证包编号，不代表第三版产品或稳定发布。今后的开发以 [transit 仓库](https://github.com/yx222yx/transit) 和 [开发约定](development.md) 为准。2026-09-07 已形成 [持续覆盖翻译与分析侧栏 Spec](specs/windows-continuous-translation.md)，作为下一轮实现依据；下文保留初版基线及长期路线，不表示新需求已完成。新功能基本实现后交用户人工检查，明确确认定版前不打包。
