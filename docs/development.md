...
当前先发布规格，再进入实现。每基本完成一个功能后，只做必要构建或针对实际失败的检查，打开开发版和相关页面交用户人工试用，根据反馈继续；没有新增修改或失败时，不反复验证。用户明确确认定版前，不运行 `-Publish` / `-Package`，不生成新的便携包，不覆盖根启动器及 `app/` 发布目录。上文构建与打包命令仅说明已有能力，不代表当前获准发布。
