...
当前使用本地英俄 OCR 的段落位置、会话内逐段文字请求与缓存，并加载简体中文 OCR 数据以辨认已有中文。纯中文不发翻译请求，混合块中的已有中文必须保留；识别置信度不足的文字保持原样并提示修正。某块译文被拒绝、截断或改写原有数值时保留该块原文，继续翻译其余块；账户或网络故障仍停止本轮请求并报错。文字区域发生变化时撤下旧译文；图片区域变化会安排本地识别，并在文字内容未变时复用请求结果。连续滚动、图片动画、长译文排版及真实页面的刷新速度仍需人工反馈，不能据编译成功宣称任意窗口兼容或达到 2–5 秒。

开发版在托盘运行时，再次双击 `scripts/start-windows-dev.cmd` 会唤回现有主窗口，不重复启动翻译会话。未配置 Key 时显示 API 配置，已配置时显示持续覆盖页。
